...
export class GameManager {
  ...
  createGame(config: GameConfig, createdBy: string): string {
  ...
  addPlayerToGame(gameId: string, clientInfo: ClientInfo, password?: string): boolean {
  ...
  removePlayerFromGame(gameId: string, clientId: string): boolean {
    const game = this.games.get(gameId);
  ...
  getPublicGames(): GameListResponse {
    const publicGames = Array.from(this.games.values())
  ...
  // Nettoyer les games inactives (optionnel)
  cleanupInactiveGames(maxAgeMs: number = 1000 * 60 * 60): number {
